...
class TestCreate:
    def test_create_qdrant(self):
        store = VectorStoreFactory.create("qdrant", use_memory=True)
...
class TestFromConfig:
    def test_from_config_qdrant(self):
        ...
        cfg = VectorDBConfig(backend="qdrant", connection_params={"use_memory": True})
    ...
    def test_from_config_defaults(self):
        ...
        cfg = VectorDBConfig()
        # Default is qdrant — may fail if no Qdrant server running,
        # so we test with explicit use_memory
        cfg_mem = VectorDBConfig(
            backend="qdrant",
            connection_params={"use_memory": True},
        )
        store = VectorStoreFactory.from_config(cfg_mem)
        assert isinstance(store, BaseVectorStore)
